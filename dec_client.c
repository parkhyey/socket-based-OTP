...
* 3. Print the decrypted text received from the server to stdout and exit the program.
*/
...
int main(int argc, char* argv[])
{
    ...
    // Add DEC_PREFIX, ciphertext, key, EOF_CHAR in order
    strcpy(output_buffer, DEC_PREFIX);
    ...
    if (argc < 4)
    {
        fprintf(stderr, "USAGE: %s ciphertext key dec_port\n", argv[0]);
        exit(0);
    }
