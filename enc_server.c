...
* 1. Accept connection from enc_client
* 2. Create a socket to listen to client requests
* 3. Spawn a child process and check for correct connection
* 4. Read messages from client and do encryption
* 5. Send the cipher text to the enc_client
*/
...
int main(int argc, char* argv[])
{
    ...
    while (1)
    {
        ...
        case 0:
            ...
            // Create a socket buffer to read in MAX_SIZE characters
            ...
            while (strstr(read_buffer, EOF_CHAR) == NULL)
            {
                ...
                // Check for prefix match to make sure it is communicating with enc_client
                if (check_prefix)
                {
            ...
            // If Connection is good, do encryption
            if (connected)
            {
                ...
                // Arrays for plain text, key, cipher text
                char plain_text[text_length + 1];
                char key[text_length + 1];
                // Add 2 for null char and EOF_CHAR
                char cipher_text[text_length + 2];

                // Set all byte to NULL terminator
                memset(plain_text, '\0', text_length + 1);
                memset(key, '\0', text_length + 1);
                memset(cipher_text, '\0', text_length + 2);

                // Copy the first half into plain text and second half into key
                strncpy(plain_text, input_buffer, text_length);
                ...
                for (int i = 0; i < text_length; i++)
                {
                    ...
                    int modulo_result = (text_i + key_i) % 27;
                    ...
                    {
                        cipher_text[i] = modulo_result + 65;
                    }
                }
                // Add EOF_CHAR at the end
                strcat(cipher_text, EOF_CHAR);

                // Send the cipher text back to the client
                charsWritten = send(connectionSocket, cipher_text, strlen(cipher_text), 0);
                if (charsWritten < 0)
                {
                    fprintf(stderr, "ERROR writing to socket\n");
                }
                if (charsWritten < strlen(cipher_text))
                {
